...
//     let highScoresService: GameHistoryService;
...
//         highScoresService = Container.get(GameHistoryService);
...
//             mock(mockPaths);
//             const highScores = await GameHistoryService['fetchDefaultHighScores']();
//             mock.restore();
...
//             const highScores = await highScoresService.getAllGameHistories();
//             expect(highScores.length).to.equal(INITIAL_HIGH_SCORES.length);
...
//             const spyFetchDefaultHighScores = stub(GameHistoryService, <any>'fetchDefaultHighScores');
